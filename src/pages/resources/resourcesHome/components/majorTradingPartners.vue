<template>
  <div class="major-trading-partners">
    <container title="主要贸易伙伴【2018.12-2021.12】">
      <el-table
        :data="tableData"
        class="table"
        stripe
      ...
      >
        ...
        <el-table-column label="采购商名称" align="center" prop="purchaser">
          <template slot-scope="scope">
            {{ getNameMap(scope.row.purchaser) }}
          </template>
        </el-table-column>
        <el-table-column
          label="交易次数"
        ...
        >
          <template slot-scope="scope">
            {{ scope.row.transactionNumber.toFixed(2) || "--" }}
          </template>
        </el-table-column>
        <el-table-column
          label="净重（千克）"
        ...
        <el-table-column label="数量" sortable align="right" prop="number">
          <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      const res = await api.getGlobalTradingPartnersSearch({
        page,
        rows,
        sort,
        order,
