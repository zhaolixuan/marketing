<template>
  <div class="import-data-statistics">
    <container title="本国从各国家/地区进口数据统计" subtitle="2018.12-2021.12">
      <el-table
        class="table"
        :data="tableData"
        stripe
      ...
      >
        ...
        <el-table-column label="国家" align="center" prop="countryOrArea">
          <template slot-scope="scope">
            {{ getNameMap(scope.row.countryOrArea) }}
          </template>
        </el-table-column>
        <el-table-column
          label="交易次数"
        ...
        <el-table-column label="数量" align="right" sortable prop="number">
          <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      const res = await api.getGlobalCountryAreaAnalysisSearch({
        page,
        rows,
        sort,
        order,
    ...
    getNameMap(name) {
      return nameMap[transformData(name)] ?? name ?? "--";
    },
  },
  watch: {
    countryOrArea: {
